...
Usually however, if you know there are going to be varsity matches that night, you can leave the streaming PC on, as well as the overhead lights.
...
Most of this process entails installing all of the software listed in the [Installing Software / Software List](#installing-software--software-list) section. Once that is done, you can install the other games that are enabled on GGLeap. Lastly, install GGLeap.

I noticed that with the new Lenovo PC's, they sometimes came with McAfee. Long story short, it's a awful antivirus software that isn't needed and absolutely should be uninstalled. The best way to do this is the McAfee removal tool, which can be found [here](https://www.mcafee.com/support/s/article/000002766?language=en_US). (Note that if you're setting up a reset PC, McAfee is most likely not installed, as it's a fresh copy of Windows.)
